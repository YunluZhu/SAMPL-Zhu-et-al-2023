...
master_root = "/Volumes/LabData/manuscript data/2022-09 VF method v2/behavior data/fly worm data"
...
for organism in ['Drosophila', 'C elegans']:
    ...
    if organism == 'Drosophila':
        ...
        for feature_toplt in features_toplot:
            ...
            plt.savefig(os.path.join(fig_dir, f"{organism}_{feature_toplt}_raw.pdf"),format='PDF')

    elif organism == 'C elegans':
        data_toplt['angle (deg)'] = 90 - data_toplt.loc[:,'angle (deg)'] # make north = 0 deg
        data_toplt = data_toplt.loc[data_toplt['time_s']<= 3/4 *data_toplt['time_s'].max()]
        set_font_type()
        features_toplot = [
                        'angle (deg)',
                    'z (mm)']
