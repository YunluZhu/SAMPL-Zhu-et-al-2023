...
def Fig8_CI_width(root):
    ...
    list_of_sample_N = np.arange(1000,len(all_feature_cond),500)
    repeated_res = pd.DataFrame()
    num_of_repeats = 20
    rep = 0
    ...
    list_of_sample_N = np.arange(2000,len(bouts_to_plot),2000)
    repeated_res = pd.DataFrame()
    num_of_repeats = 20
    rep = 0
    ...
    while rep < num_of_repeats:
        list_of_ci_width = []
        for sample_N in list_of_sample_N:
            sample_for_fit = bouts_to_plot.sample(n=sample_N)
