...
for excluded_exp, idx_group in enumerate(idx_list):
    ...
    pitch_angSpd_max = group.loc[group.idx==idx_angSpd_max,'propBoutAligned_pitch'].values
    ...
    # rot_total_accel = pitch_peak - pitch_initial
    # rot_150_25_accel = pitch_pre_25 - pitch_pre_150
    rot_ang_accel = pitch_angSpd_max - pitch_initial
    rot_narrow_ang_accel = pitch_angSpd_max - pitch_pre_bout
    ...
    bout_features = pd.DataFrame(data={'pitch_pre_bout':pitch_pre_bout,
                                        'rot_ang_accel':rot_ang_accel,
                                        'rot_narrow_ang_accel': rot_narrow_ang_accel,
    ...
    corr_calc = this_dpf_res.groupby(grp_cols).apply(
        lambda y: stats.pearsonr(
            y['pitch_peak'].values,y['propBoutAligned_angVel'].values)[0]
            )
    corr_calc.name = 'corr_peakPitch'
    ...
    corr_calc.name = 'corr_traj_deviation'
    corr_traj_deviation = corr_calc
      
    corr_calc = this_dpf_res.groupby(grp_cols).apply(
        lambda y: stats.pearsonr(
            y['bout_traj_peak'].values,y['propBoutAligned_instHeading'].values)[0]
            )
    ...
    corr_calc = this_dpf_res.groupby(grp_cols).apply(
        lambda y: stats.pearsonr(
            y['bout_traj_peak'].values,y['propBoutAligned_pitch'].values)[0]
            )
    corr_calc.name = 'corr_pitch_trajPeak'
    corr_pitch_traj = corr_calc.to_frame()
    # -------
    corr_this_exp = corr_angvel
    corr_this_exp = corr_this_exp.join(corr_atkAng)
    corr_this_exp = corr_this_exp.join(corr_traj_deviation)
    corr_this_exp = corr_this_exp.join(corr_instTraj)
    corr_this_exp = corr_this_exp.join(corr_pitch_traj)
    corr_this_exp = corr_this_exp.reset_index()
    corr_this_exp = corr_this_exp.assign(
                                    jackknife_num = np.mean(idx_group),
                                    )
    corr_all = pd.concat([corr_all,corr_this_exp])
    ...
    # corr_angvel = corr_angvel.join(corr_traj_deviation)
...
corr_pitch_traj = corr_pitch_traj.reset_index(drop=False)
...
for corr_which in ['corr_traj_deviation','corr_peakPitch','corr_pitch_trajPeak']:
    g = sns.relplot(
        # hue='condition',
        x='time_ms',
        y=corr_which,
    ...
    if corr_which=='corr_traj_deviation':
        print("Figure 4: corr. of angVel with traj. deviation")
        g.set_ylabels(f'angvel {corr_which}')
        plt.savefig(fig_dir4+f"/angvel {corr_which}.pdf",format='PDF')
    elif corr_which=='corr_peakPitch':
        print("Figure 6: corr. of angVel with pre_bout_pitch")
        g.set_ylabels(f'angvel {corr_which}')
        plt.savefig(fig_dir6+f"/angvel {corr_which}.pdf",format='PDF')

# %%
# which_rotation = 'rot_ang_accel'
# which_rotation = 'rot_l_accel'
...
# # %%
# # plot up down separated
# upper = np.percentile(toplt[which_rotation], 98)
# lower = np.percentile(toplt[which_rotation], 2)
# xlabel = "Rotation from initial rot (deg)"
# ylabel = 'Trajectory deviation (deg)'
 
# g = sns.relplot(
#     kind='scatter',
#     data = to_plt.sample(n=1800),
#     x = which_rotation,
#     y = 'traj_deviation',
#     # x_bins=np.arange(int(lower),int(upper),3),
#     # x_ci=95,
...
# g.set(ylim=(-15,25))
# g.set(xlim=(lower,upper))
# g.map(sns.lineplot,data=binned_df_dir,
#       hue='direction',
#       x='Accel rotation',
#       y='traj_deviation',
#       style='direction')
# g.set_axis_labels(x_var = xlabel, y_var = ylabel)
# sns.despine()
# plt.savefig(fig_dir4+f"/traj deviation vs {which_rotation} by_dir.pdf",format='PDF')
# r_val = toplt.groupby('direction').apply(
#     lambda x: stats.pearsonr(x[which_rotation],x['traj_deviation'])[0]
# )
# print(f"pearson's r = {r_val}")
# # %%
# # plot up down separated
...
# # g.set_axis_labels(x_var = xlabel, y_var = ylabel)
# sns.despine()
# plt.savefig(fig_dir4+f"/traj deviation vs {which_rotation} by_dir_regression.pdf",format='PDF')
# r_val = toplt.groupby('direction').apply(
#     lambda x: stats.pearsonr(x[which_rotation],x['traj_deviation'])[0]
# )
# print(f"pearson's r = {r_val}")
