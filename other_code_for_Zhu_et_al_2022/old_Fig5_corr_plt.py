'''
plot correlation of angular velocity with:
1. pre bout posture 
2. attack angle
...as a function of time

no jackknife, generates one list of corr values per exp repeat
mean±sd
'''
...
folder_name = f'{pick_data} angvel corr accel decel'
...
for excluded_exp, idx_group in enumerate(idx_list):
    ...
    bout_features = pd.DataFrame(data={'pitch_pre_bout':pitch_pre_bout,
                                       ...
                                       'atk_ang': epochBouts_trajectory-pitch_pre_bout,
                                       'traj_residual':epochBouts_trajectory-pitch_initial,
    ...
    this_dpf_res = this_dpf_res.assign(
                                ...
                                traj_residual = this_dpf_res['bout_traj']-this_dpf_res['pitch_initial'],
                                )
    ...
    corr_calc = this_dpf_res.groupby(grp_cols).apply(
        lambda y: stats.pearsonr(
            y['traj_residual'].values,y['propBoutAligned_angVel'].values)[0]
            )
