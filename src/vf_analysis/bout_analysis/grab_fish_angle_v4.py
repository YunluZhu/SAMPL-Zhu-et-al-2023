...
def grab_fish_angle(analyzed, fish_length,sample_rate):
    ...
    for i, bout in bout_idx.iterrows():
        ...
        # bout['swim_end_idx']-bout['swim_start_idx'] > frame_number25: # added by YZ, get rid of bouts with only 25ms above speed threshold
    ...
    for i, row in IEI_attributes.iterrows():
        ...
        bout_end_post = row['swim_end_shift'] + 1 + POST_BOUT_BUF  # where last bout ends. to match the swim end in Matlab, +1, 100ms after swim ends (speed lower than threshold)
        bout_start_pre = row['swim_start_idx'] - IEI_2_swim_buf  # 100ms before next bout starts
